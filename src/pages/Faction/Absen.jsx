...
const Absen = () => {
	...
	React.useEffect(()=>{
		const getEvents = async ()=> {
			const response = await axios.get(`${BACKEND_URL}/event/?user_id=${user.id}`, 
			{
			headers:{
	...
	const absen = (event_id) => () => {
		...
		const response = await axios.post(`${BACKEND_URL}/absen`, {
			userId:user.id,
			eventId:event_id
		},
		{
		...
		if(response.status === 200){
			...
			const absens = await axios.get(`${BACKEND_URL}/event/?user_id=${user.id}`, 
			{
			headers:{
				Authorization: `Bearer ${localStorage.getItem("auth")}`
			}      
      ...
      <Swiper {...params}>
	      {
		      events.map(event=>(
			      ...
			      ?
		<Box>
			<hr style={{backgroundColor:"white", height:"0.25rem", width:"90%", margin:"1rem auto", borderRadius:"25px"}}/>
			<Text type="secondary" color="white" size={1.5}>{event.event_name}</Text>
			<br/>
			...
			<br/>
			<br/>
			<Button style={{position:"absolute", right:"1rem", bottom:"1rem"}} variant="contained" color="light"
